...
int main( int argc, char *argv[ ] )
{
	...
	if (rank == 0)
	{
		...
		{
			int x = rand() % noOfElems;
			int y = rand() % noOfElems;
	...
	double *T = new double[noOfElems];

	cilk_pm_sort(T, localData, 0, localDataLen - 1);
	...
	for (i = 0; i < numOfProc; i++)
		//MPI_Alltoall(buckets[i],localDataLen,MPI_DOUBLE,finalBucket,localDataLen,MPI_DOUBLE,MPI_COMM_WORLD);
	...
	T = new double[noOfElems];
	cilk_pm_sort(T, finalData, 0, k - 1);
	delete []T;
